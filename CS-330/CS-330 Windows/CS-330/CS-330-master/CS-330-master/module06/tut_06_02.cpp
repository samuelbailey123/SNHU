...
public:
	...
	{
		brick_type = bt; x = xx; y = yy, width = ww; red = rr, green = gg, yellow = bb;
		onoff = ON;
	};
	...
	void drawBrick()
	{
		if (onoff == ON)
		{
			double halfside = width / 1.1;

			glColor3d(red, green, yellow);
			glBegin(GL_POLYGON);
...
int main(void) {
	...
	Brick brick2(REFLECTIVE, -0.5, 0.33, 0.2, 0, 1, 0);
	Brick brick3(REFLECTIVE, -0.5, -0.33, 0.2, 0, 1, 1);
