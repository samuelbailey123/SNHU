...
bool run_query(sqlite3* db, const std::string& sql, std::vector<user_record>& records){
  ...
  //regex method
  std::regex assignment("[^0-9A-Z+$]\\s*(and|or)\\s*[\\'\"]{0,1}", std::regex_constants::icase);
  if(std::regex_search(sql, assignment)){
    std::cout << "error" << std::endl;
    return false;
  }

  //NON regex method
  // if((sql.find("=") == std::string::npos && sql.find("OR") == std::string::npos || sql.find("or") == std::string::npos)){
  //   std::cout << "error" << std::endl;
  //   return false;
  // }
