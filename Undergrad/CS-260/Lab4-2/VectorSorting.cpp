...
void selectionSort(vector<Bid>& bids){
    ...
    for (int i = 0; i<bids.size(); i++){
        ...
        for (int j = 0; j<bids.size(); j++){
            if (bids.at(j).title.compare(bids.at(lowest).title) < 0){
                lowest = j; 
            }
        }
        if (lowest != i) {
			swap(bids.at(i), bids.at(lowest));
		}
    }
}
...
int main(int argc, char* argv[]) {
    ...
    while (choice != 9) {
        ...
        case 4: 
            // I couldn't find the exact way it was suppose to look in the txt file so I made it similar to case 3
            ticks = clock(); 
